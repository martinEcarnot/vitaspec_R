...
print(f"exec XGB_moy pour: {compose}")
...
# save
chemin_test_externe = d0 / "xgboost" / "jeux_test" / idparam / f"valid_externe_{compose}.csv"
...
if meilleur_modele_joblib is not None:
    print(f"\n� meilleur mod XGB (RMSECV: {meilleur_rmsecv_global:.4f})")
    ...
    chemin_modele = dossier_compose / f"modele_XGB_A_{compose}.joblib"
        ...
        try:
            with plt.style.context('default'):
                ...
                if len(x_values) != len(importances):
                    print(f"({len(x_values)} longueurs ondes pour {len(importances)} variables)")
