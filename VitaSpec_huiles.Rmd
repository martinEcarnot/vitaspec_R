...
# CAROT=read.table(paste0(d0,"/Résutats_VitaPalm_Amel_Joualli_Caroténoïdes.csv"), sep=";", header=TRUE, dec=",")  # Echantillons mélangés
CAROT=read.table(paste0(d0,"/carotenes révisés février 2025.csv"), sep=",", header=TRUE, dec=".")
...
vitaspec_load=function(d0,d1) {
  ...
  sp=asd_read_dir(d)
  sp=sp2df(sp)
  colnames(sp)[1]=paste0("x",sub(".*_(.*)$", "\\1", d1))
  sp$ech=substr(rownames(sp$x),1,nchar(rownames(sp$x))-5)
  sp$spname=rownames(sp$x)
  return(sp)
}
  
spAD=merge(vitaspec_load(d0,"20240424_VitaSPEC_HR_AGAP"),vitaspec_load(d0,"20240424_VitaSPEC_HR_DIADE"),by="spname")
...
load('vitaspec_oil')
sp$x=sp$xDIADE;
sp$TT=sp$aT3+sp$gT3+sp$dT3 # sp$aT+sp$aT3+sp$gT3+sp$dT3
ag=colnames(sp)[18] #[c(13:16,19)]
options(warn=-1)
...
load('vitaspec_oil')
sp$x=sp$xAGAP
spred=vitaspec_load("20241209_Vitaspec_HR_AGAP") 
AG2=read.table(paste0(d0,"Matrice Acide gras- Huile de palme rouges Amel Joualli.csv"), sep=";", header=TRUE, dec=",")
LVA=8 #c(7,8,4,7,4,7)
