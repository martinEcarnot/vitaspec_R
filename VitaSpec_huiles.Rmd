...
AGL=read.table(paste0(d0,"analyses_ref/","teneur en acides gras libres.csv"), sep=";", header=TRUE, dec=",")[,-1]
...
rbind(list('adj',''),list('red',c(1,1180,1)),list('snv',''),list('sder',c(2,3,51))),
...
for (j in 1:length(list_pre)) {  # 1:
  ...
  iout=which(dat$x[,1650] < 0.1 | is.na(dat[,colnames(dat)==ag1])) # which(fm1$fit$y1 < 0) # Probablement un échantillon contenant de l'eau  # | dat[,colnames(datok)==ag1] > 100
