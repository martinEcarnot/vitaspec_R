...
library(plyr)
library(nirsextra)
AG=read.table("/home/ecarnot/Documents/INRA/Projets/VitaSPEC/Matrice Acides Gras Amel_avril2024.csv", sep=";", header=TRUE, dec=",")
CAROT=read.table("/home/ecarnot/Documents/INRA/Projets/VitaSPEC/Résutats_VitaPalm_Amel_Joualli_Caroténoïdes.csv", sep=";", header=TRUE, dec=",")
TOCO=read.table("/home/ecarnot/Documents/INRA/Projets/VitaSPEC/Résultats Vitaspec_Amel Joualli_Tocochromanols ( vitamine E).csv", sep=";", header=TRUE, dec=",")
AGL=read.table("/home/ecarnot/Documents/INRA/Projets/VitaSPEC/teneur en acides gras libres.csv", sep=";", header=TRUE, dec=",")[,-1]
...
vitaspec_merge=function(ASD) {
  ...
  sp=asd_read_dir(d)
    ...
    sp=merge(vitaspec_merge("AGAP"),ref,by="ech")
    sp=merge(vitaspec_merge("DIADE"),sp,by="ech")
    
        spm=merge(vitaspec_merge("AGAP"),merge(vitaspec_merge("DIADE"),ref,by="ech"),by="ech")

    spj = join_all(list(vitaspec_merge("AGAP"),vitaspec_merge("DIADE"),ref), by = 'ech', type = 'inner')
    sp$rep=as.numeric(substr(rownames(sp$xAGAP),nchar(rownames(sp$xAGAP))-4,nchar(rownames(sp$xAGAP))))
...
sp$rep=as.numeric(substr(rownames(sp$x),nchar(rownames(sp$x))-4,nchar(rownames(sp$x))))
...
# chart.Correlation(ref[,-1])
return(list(sp=sp,list_pre=list_pre, ref=ref))
}
```
...
```{r}
x=vitaspec_load("~/Documents/INRA/Projets/VitaSPEC/20240424_VitaSPEC_HR_DIADE/") # 20240424_VitaSPEC_HR_DIADE/
sp=x$sp; list_pre=x$list_pre; ref=x$ref[,c(10:14,16)]
ag=colnames(ref)#[-1]
options(warn=-1)
